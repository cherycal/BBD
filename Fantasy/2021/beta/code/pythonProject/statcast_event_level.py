		...
		if table_exists(tablename):
			...
			if tablerows == 0:
				...
				if df_rows == tablerows and override is False:
					...
					delete_cmd = "DELETE from " + tablename + " where gamedate = " + dt8
...
def get_one_day(f, dt_, year_, player_type, print_header=0):
	...
	# r = requests.get(url_details_text, allow_redirects=True)
	# outfile = "C:\\Users\chery\Documents\BBD\Statcast\\" + player_type + str(year_) + "_events_daily.csv"
...
def single_year(year, bp, dates=(3, 22, 10, 4)):
	###################
	###################
	# Hard coded:
	sleep_interval = .5
	...
	# Cleanup
	bdb.update(
		"update StatcastBattingEvents set xBA = 0, xwOBA = 0, xSLG = 0, adj_xwOBA = 0, adj_xSLG = 0 where xwOBA is NULL and Result like 'strikeout%'")
	bdb.update(
		"update StatcastPitchingEvents set xBA = 0, xwOBA = 0, xSLG = 0, adj_xwOBA = 0, adj_xSLG = 0 where xwOBA is NULL and Result like 'strikeout%'")
	bdb.update(
		"update StatcastBattingEvents set xwOBA = 0.69, xSLG = 1, adj_xwOBA = 0.69, adj_xSLG = 1 where xwOBA is NULL and Result = 'walk'")
	bdb.update(
		"update StatcastPitchingEvents set xwOBA = 0.69, xSLG = 1, adj_xwOBA = 0.69, adj_xSLG = 1 where xwOBA is NULL and Result = 'walk'")
...
def main_old():
	...
	year = str(season)
	raw_outfile = "C:\\Users\\chery\\Documents\\BBD\\Statcast\\" + bpdir + "\\" + year + bp + "_events_raw.csv"
	...
	outfile = "C:\\Users\chery\Documents\BBD\Statcast\\" + bpdir + "\\" + year + bp + "_events_daily.csv"
	post_process_csv_file(raw_outfile, outfile)

	table_name = "Statcast" + bpdir + "Events"
