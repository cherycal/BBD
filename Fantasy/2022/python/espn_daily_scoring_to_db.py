                    ...
                    for entry in entries:
                        ...
                        if len(entry['playerPoolEntry']['player']['stats']) > 0:
                            for game in entry['playerPoolEntry']['player']['stats']:
                                lol.clear()
                                game_id = game.get('externalId', 0)
                                # print(f"gameid: {game_id}")
                                player_stats = game['stats']
                                cols = ['Name', 'playerid', 'Date', 'gameid', 'leagueId',
                                        'teamId', 'lineupSlotId', 'points']
                                # vals = ['', 0, 0, 0, 0, 0, None]
                                vals = [player_name, player_id, date8, game_id, league, team_id,
                                        fantasy.get_position(lineup_slot), 0]
                                # vals[0] = player_name
                                # vals[1] = player_id
                                # vals[2] = date8
                                # vals[3] = league
                                ...
                                for stat in player_stats:
                                    ...
                                    if stat_int in statid_dict:
                                        ...
                                        # vals[5] = fantasy.get_position(lineup_slot)
                                        # vals[6] = points
                                        vals[0:8] = [player_name, player_id, date8, game_id, league, team_id,
                                ...
                                if entryhasstats or True:
                                    ...
                                    while incomplete and tries < max_tries:
                                        ...
                                        # print(df)
                                            ...
                                            print(f"str({ex}): {table_name}, {str(date8)}")
                                            # print(df)
                                            tries += 1
                                    if tries >= max_tries:
