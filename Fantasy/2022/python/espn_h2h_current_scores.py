...
def get_points_page( sleep_interval, matchupPeriodId, season, teamId, leagueId):
	...
	try:
		...
		msg = f'{teams[0].text} {scores[0].text}, {teams[1].text} {scores[1].text} ({diff})\n'

		inst.push("SENZ score", msg)
		inst.tweet(msg)
...
def get_cats_page( sleep_interval, matchupPeriodId, season, teamId, leagueId):
	...
	try:
		...
		if len(results) > 3:
			msg = f'{results[0].text} {results[1].text}, {results[2].text} {results[3].text}\n'
