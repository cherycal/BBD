...
def delete_rosters():
    bdb.delete("DELETE from ESPNRosters")
            ...
            if 'roster' in team:
                for player in team['roster']['entries']:
                    ...
                    if not old_rosters.get(k):
                        ...
                        insert_list_many.append((player_full_name, team_name, str(league), str(espn_id), pos))
...
def run_inserts():
    ...
    #     print(command)
    #     bdb.insert(command)
    bdb.insert_many("ESPNRosters", insert_list_many)
...
def send_msg(msg_list):
    ...
    inst.push("Roster changes at " + str(date_time), str(len(msg_list)) + " entries")
